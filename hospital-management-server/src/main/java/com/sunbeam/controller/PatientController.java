...
//	@PreAuthorize("hasRole('ROLE_PATIENT' )")
	...
	public void updatePatientPaymentStatus(@RequestBody PatientDataBacking patientData) {
		...
		patientData= pServices.getPatientById(patientData.getPatId());
		if(patientData.getPaymentStatus().equals("paid"))
		{
			pServices.addPayementDate(patientData.getPatId());
		
		emailSenderService.sendEmail(patientData.getEmail(), "Payement Success", "Thank You "+patientData.getFirstName()+" "+patientData.getLastName()+" "+"For Your Payment");
		}
		else
		{
			emailSenderService.sendEmail(patientData.getEmail(), "Payement Pending", "Dear "+patientData.getFirstName()+" "+patientData.getLastName()+" "+"Kindly Do Payment");
