...
public interface IPatientDao extends JpaRepository<Patient,Integer> {
	...
	@Query(value = "insert into patients values (:id, :user_id, :ward_id, :doctor_id, :date_of_admission"
			+ ", :blood_group, :dob, :prescription, :bed_alloted, :payment_status, :patient_problem,null)",nativeQuery = true)
...
//	select datediff(date_of_admission,date(now())) from patients where id=37;
	...
	@Modifying
	@Query(value="update patients set paymentDate=date(now()) where id=:patid", nativeQuery = true)
	void addPaymentDao(@Param("patid")int patId);
}
